...
export function AdminGroupsListPage() {
  ...
  const fetchList = useCallback(
    async (query: ObjectListQuery) => {
      ...
      return api.adminGroupsList(vaultId, {
        view: query.view,
        navigationContext: ADMIN_GROUPS_ENTRY_CONTEXT,
  ...
  const persistView = useCallback(
    async (viewId: string) => {
      if (!vaultId) return;
      await api.saveAdminGroupsListView(vaultId, {
        view_id: viewId,
        navigation_context: ADMIN_GROUPS_ENTRY_CONTEXT,
      });
    },
    [vaultId],
  );
  ...
  const persistColumnWidths = useCallback(
    async (nextWidths: Record<string, number>) => {
      ...
      try {
        await api.saveAdminGroupsListGridPreference(vaultId, {
          navigation_context: ADMIN_GROUPS_ENTRY_CONTEXT,
          grid_preferences: {
  ...
  return (
    <AdminPageShell variant="list" title={displayText(model?.tab_label, "Groups")}>
        {model && (
          <AdminObjectListToolbar
            ...
            createButton={
              <ListCreateButton
                vaultId={vaultId}
                tabApiName={ADMIN_GROUPS_TAB}
                objectApiName={ADMIN_GROUPS_OBJECT}
