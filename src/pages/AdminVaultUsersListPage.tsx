...
import {
  ADMIN_VAULT_USERS_ENTRY_CONTEXT,
  ADMIN_VAULT_USERS_OBJECT,
  ADMIN_VAULT_USERS_TAB,
} from "../lib/adminVaultUsers";
import { OBJECT_LIST_PAGE_SIZE_OPTIONS, type ObjectListQuery } from "../lib/objectListPage";

export function AdminVaultUsersListPage() {
  ...
  const fetchList = useCallback(
    async (query: ObjectListQuery) => {
      ...
      return api.adminVaultUsersList(vaultId, {
        view: query.view,
        navigationContext: ADMIN_VAULT_USERS_ENTRY_CONTEXT,
  ...
  const persistView = useCallback(
    async (viewId: string) => {
      if (!vaultId) return;
      await api.saveAdminVaultUsersListView(vaultId, {
        view_id: viewId,
        navigation_context: ADMIN_VAULT_USERS_ENTRY_CONTEXT,
      });
    },
    [vaultId],
  );
  ...
  const {
    ...
    scopeKey: ADMIN_VAULT_USERS_TAB,
    vaultId,
  ...
  const persistColumnWidths = useCallback(
    async (nextWidths: Record<string, number>) => {
      ...
      try {
        await api.saveAdminVaultUsersListGridPreference(vaultId, {
          navigation_context: ADMIN_VAULT_USERS_ENTRY_CONTEXT,
          grid_preferences: {
  ...
  return (
    <AdminPageShell variant="list" title={displayText(model?.tab_label, "Vault Users")}>
        {model && (
          <AdminObjectListToolbar
            ...
            createButton={
              <ListCreateButton
                vaultId={vaultId}
                tabApiName={ADMIN_VAULT_USERS_TAB}
                objectApiName={ADMIN_VAULT_USERS_OBJECT}
